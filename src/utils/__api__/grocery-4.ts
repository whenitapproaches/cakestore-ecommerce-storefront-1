...
const getBreadcrumb = cache(async (slug?: string): Promise<string> => {
  const response = await axios.get("/api/grocery-4/breadcrumb", { params: { slug } });
  return response.data;
});

export default { getAllProducts, getStories, getCategories, getBreadcrumb, getAllProductsBySlug };
